...
class NotEqToConstant(Predicate):
    ...
    def evaluate(self, log: pd.DataFrame) -> pd.DataFrame:
        return pm4py.filter_event_attribute_values(log, self.attribute_key, self.values, level="case", retain=False)
    ...
    def as_string(self) -> str:
        ...
        return f'({self.attribute_key} not in {{ {values_as_string} }})'
