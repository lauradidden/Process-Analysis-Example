...
class LessEqualToConstant(Predicate):
    ...
    def evaluate(self, log: pd.DataFrame) -> pd.DataFrame:
        ...
        i1 = log.set_index(LessEqualToConstant.case_id_glue).index
        i2 = filtered_df_by_ev.set_index(LessEqualToConstant.case_id_glue).index
        ret = log[i1.isin(i2)]
